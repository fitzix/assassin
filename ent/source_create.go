...
func (sc *SourceCreate) SetProviderID(id int) *SourceCreate {
	if sc.provider == nil {
		sc.provider = make(map[int]struct{})
	}
	sc.provider[id] = struct{}{}
	return sc
}

// SetProvider sets the provider edge to Provider.
func (sc *SourceCreate) SetProvider(p *Provider) *SourceCreate {
...
func (sc *SourceCreate) Save(ctx context.Context) (*Source, error) {
	...
	if len(sc.provider) > 1 {
		return nil, errors.New("ent: multiple assignments on a unique edge \"provider\"")
	}
	if sc.provider == nil {
		return nil, errors.New("ent: missing required edge \"provider\"")
	}
	if len(sc.version) > 1 {
...
func (sc *SourceCreate) sqlSave(ctx context.Context) (*Source, error) {
	...
	if nodes := sc.provider; len(nodes) > 0 {
