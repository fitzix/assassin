...
func (tq *TagQuery) QueryApp() *AppQuery {
	query := &AppQuery{config: tq.config}
...
func (tq *TagQuery) WithApp(opts ...func(*AppQuery)) *TagQuery {
	query := &AppQuery{config: tq.config}
	...
	if query := tq.withApp; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.AppPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "app": %v`, err)
		}
